...
Kontrakt providera:
    generate(images, prompt, cheap=False) -> (text, usage {"in","out"}, model)
      images: list[bytes]  (PNG)
      cheap=True  → lacný/rýchly model (výber strany pôdorysu a pod.)
      cheap=False → hlavný vision model (extrakcia)
...
class GeminiVision:
    ...
    def generate(self, images, prompt, cheap=False):
        ...
        parts.append(prompt)
        cfg = t.GenerateContentConfig(thinking_config=t.ThinkingConfig(
